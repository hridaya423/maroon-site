...
import { Ship, Compass, Anchor, Terminal, Sword, Map, Filter, Code } from 'lucide-react';
import {
  Accordion,
  AccordionContent,
  AccordionItem,
  ...
  const v2examples = [
    {
      ...
      icon: Filter,
      description: "Advanced list operations with map, filter, and reduce",
      code: `# Advanced Data Processing Example
crew_data be list of 
    {"name": "Jack", "exp": 5, "skill": 8},
    {"name": "Anne", "exp": 3, "skill": 9},
    {"name": "Mary", "exp": 7, "skill": 7}

# Filter for experienced crew
voyage is_experienced(pirate):
    return pirate["exp"] greater_than 4
end voyage

# Map to calculate effectiveness
voyage calculate_power(pirate):
    return pirate["exp"] times pirate["skill"]
end voyage

# Find experienced crew
veterans be filter sails with crew_data, "is_experienced"
bark "Veterans:", veterans

# Calculate crew power
crew_power be map sails with crew_data, "calculate_power"
bark "Crew Power:", crew_power

# Total crew experience
total_exp be reduce crew_data with result plus it["exp"]
bark "Total Experience:", total_exp`
    },
    {
      title: "Enhanced Error Handling",
      icon: Code,
      description: "Complex error handling with try-catch and switch statements",
      code: `# Enhanced Error Handler
voyage dangerous_operation(risk_level):
    brace for impact:
        choose risk_level:
            case "low": 
                success_rate be 90
            ...
            case "high": 
                success_rate be 30
            default: 
                throw "Invalid risk level!"
        end choose

        roll be random sails with 1, 100
        if roll be greater_than success_rate:
            throw "Operation failed with " + (100 minus roll) + " margin"
        end voyage

        bark "Operation successful!"
        return true

    if capsized:
        bark "Error caught:" capsized
        return false
end voyage

# Test different risk levels
risk_levels be list of "low", "medium", "high"
plunder each risk from risk_levels:
    bark "\nTesting risk level:" risk
    dangerous_operation sails with risk`
    }
  ];
  const examples = [
    {
...
repeat 5 times:
  ...
  return (
    ...
    <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-white mb-4">Core Examples</h2>
        <Accordion type="single" collapsible className="space-y-4">
